    ...
    for row in rows:
        ...
        if not sector_type and label:
            ...
            if "UMD" in up or "MALADES DIFFICILES" in up:
                ...
                "INFANTO" in up or "PÉDOPSY" in up or "PEDOPSY" in up or "ENFANT" in up or "ADOLESCENT" in up
            ):
                sector_type = "I"
...
def parse_structure(filepath: str) -> dict:
    ...
    mapping = _normalize_header(headers) if headers else _normalize_header(list(_FALLBACK_ORDER))
...
def _draw_org_node(pdf, node: dict, x: float, y: float, node_w: float, node_h: float, scale: float) -> None:
    """
    ...
    pdf.rect(x, y, node_w, node_h, style="FD", round_corners=True, corner_radius=max(1.0, 2.0 * scale))
...
def _layout_tree(roots: list[dict], node_w: float, node_h: float, h_gap: float, v_gap: float) -> dict:
    """
...
def _draw_organigram_page(pdf, subtree_roots: list[dict], title: str, subtitle: str = "") -> None:
    """
...
def render_tree_pdf(parsed: dict, output_path: str) -> str:
    ...
    class Org(FPDF):
        ...
        def header(self):
            ...
            self.cell(0, 7, f"Structure : {filename}", new_x="RIGHT", new_y="TOP")
            self.set_font("Helvetica", "", 8)
            self.set_text_color(100, 116, 139)
            self.cell(0, 7, date.today().isoformat(), new_x="LMARGIN", new_y="NEXT", align="R")
        ...
        def footer(self):
            ...
            self.cell(0, 6, f"Sovereign OS DIM - Page {self.page_no()} / {{nb}}", align="C")
    ...
    if by_level:
        ...
        for lvl, count in by_level.items():
            ...
            pdf.cell(30, 5, f" {lvl.upper()} : {count} ", new_x="RIGHT", new_y="TOP", fill=True)
    ...
    if by_sector:
        ...
        for stype, count in by_sector.items():
            ...
            pdf.cell(34, 5, f" {short} : {count} ", new_x="RIGHT", new_y="TOP", fill=True)
            pdf.cell(1.5, 5, "", new_x="RIGHT", new_y="TOP")
        pdf.ln(8)
    ...
    for parent, sub in page_targets:
        ...
        subtitle_bits = []
        if parent:
            subtitle_bits.append(f"Rattache a {parent.get('code', '')} ({parent.get('label', '')})")
