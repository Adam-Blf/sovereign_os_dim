...
ATIH_FORMAT_VARIANTS = {
    "RPS": [
        ...
        {"length": 142, "ipp": (21, 41), "ddn": (41, 49), "label": "RPS-P04-142 (ancien 2021)"},
        {"length": 148, "ipp": (21, 41), "ddn": (41, 49), "label": "RPS-P04-148 (transition 2021)"},
    ],
    "RAA": [
        # Format ancien RAA : ligne de 86 ou 90 chars au lieu de 96
        {"length": 86, "ipp": (21, 41), "ddn": (41, 49), "label": "RAA-ancien-86 (2021)"},
        {"length": 90, "ipp": (21, 41), "ddn": (41, 49), "label": "RAA-ancien-90 (transition 2021)"},
    ],
}
...
class DataProcessor:
    ...
    __slots__ = ("matrix", "mpi", "file_stats", "processed_files", "logs", "_progress_cb", "_variant_cache")
    ...
    def _process_single_file(self, finfo: dict) -> dict:
        ...
        if stats["ipp_normalized"] > 0:
            self._log(f"🔧 {finfo['name']} : {stats['ipp_normalized']} IPP normalisés (cohérence BIQuery)")
    ...
    def process_files(self, file_list: list) -> dict:
        ...
        if len(file_list) == 1:
            ...
            with ThreadPoolExecutor(max_workers=workers) as pool:
                futures = {pool.submit(self._process_single_file, f): f for f in file_list}
        ...
        for res in results:
            ...
            self.file_stats[res["name"]] = {"format": res["format"], **s}
    ...
    def get_collisions(self) -> list:
        ...
        for ipp, data in self.mpi.items():
            if len(data["history"]) > 1:
                opts = sorted(
                    [{"ddn": d, "count": len(s), "sources": s[:5]} for d, s in data["history"].items()],
    ...
    def auto_resolve_all(self) -> int:
        ...
        for ipp, data in self.mpi.items():
            if len(data["history"]) > 1 and data["pivot"] is None:
                best = max(data["history"].keys(), key=lambda d: (len(data["history"][d]), d))
                data["pivot"] = best
                n += 1
    ...
    def export_csv(self, file_list: list, output_dir: str) -> dict:
        ...
        generated = []
        stats = {"csv_count": 0, "lines_exported": 0, "ddn_corrected": 0, "files_skipped": 0}
        ...
        for finfo in file_list:
            ...
            try:
                with (
                    open(finfo["path"], "r", encoding="latin-1", errors="replace") as fi,
                    ...
                    for raw in fi:
                        ...
                        if ipp in self.mpi and self.mpi[ipp]["pivot"]:
                            ...
                            if line[d0:d1].strip() != piv:
                                ddn_len = d1 - d0
                                line = line[:d0] + piv.ljust(ddn_len) + line[d1:]
                                ddn_out = piv
    ...
    def export_sanitized_txt(self, filepath: str, output_dir: str) -> dict:
        ...
        try:
            with (
                open(filepath, "r", encoding="latin-1", errors="replace") as fi,
                ...
                for raw in fi:
                    ...
                    if ipp in self.mpi and self.mpi[ipp]["pivot"]:
                        piv = self.mpi[ipp]["pivot"]
                        if line[d0:d1].strip() != piv:
                            line = line[:d0] + piv.ljust(d1 - d0) + line[d1:]
                            stats["pivoted"] += 1
    ...
    def compute_active_population(self) -> dict:
        ...
        by_year_field: dict[str, dict[str, set]] = defaultdict(lambda: defaultdict(set))
        by_year_format: dict[str, dict[str, set]] = defaultdict(lambda: defaultdict(set))
        ...
        fields = sorted({f for per_year in by_year_field.values() for f in per_year})
        ...
        return {
            ...
            "by_year_field": {
                y: {f: len(ipps) for f, ipps in per_field.items()} for y, per_field in by_year_field.items()
            },
            "by_year_format": {
