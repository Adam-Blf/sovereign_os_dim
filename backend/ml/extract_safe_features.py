        ...
        with open(path, "r", encoding="latin-1", errors="replace") as f:
            for i, line in enumerate(f):
                ...
                if n >= 3 and line[0:3].startswith("H"):
                    has_had_token += 1
                # Double FINESS
                if n >= 18 and line[0:9].strip().isdigit() and line[9:18].strip().isdigit():
                    has_double_finess += 1
...
def main() -> None:  # pragma: no cover
    ...
    p.add_argument(
        "--max-lines", type=int, default=10_000, help="Limite de lignes lues par fichier (défaut 10k)."
    )
    args = p.parse_args()
    ...
    for path in files:
        ...
        rows.append(row)
        print(f"       {fmt:<24} {os.path.basename(path)[:60]:<60} {stats.get('n_lines', 0):>6} lignes")
