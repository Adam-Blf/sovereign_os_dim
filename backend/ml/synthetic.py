...
def _build_line(spec: FormatSpec, rng: random.Random, inject_ddn_error: bool = False) -> tuple[str, str]:
    """
    ...
    if spec.ddn_start is not None:
        if inject_ddn_error and rng.random() < 0.5:
            ddn = rng.choice(
                ["00000000", "99999999", "20251332", " " * 8, "abcdefgh", "20250230", "12131980"]
            )
            ddn_valid = "0"
        else:
...
def generate_dataset(
    n_samples: int = 50_000, seed: int = 42, ddn_error_rate: float = 0.08, collision_rate: float = 0.06
) -> pd.DataFrame:
    """
    ...
    for i in range(n_samples):
        ...
        row = {f"feat_{k}": v for k, v in {**line_feats, **mpi_feats}.items()}
