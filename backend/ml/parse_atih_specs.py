...
class FormatSpec2026:
    ...
    def ipp(self) -> FieldSpec | None:
        return self._find_field(
            ["IPP", "Identifiant Permanent du Patient", "N� d'identification permanent du patient"]
        )
    ...
    def finess_geo(self) -> FieldSpec | None:
        return self._find_field(["FINESS g�ographique", "FINESS geographique", "N� FINESS g�ographique"])
...
def _is_field_header_row(row: tuple) -> bool:
    ...
    return (
        ("libell" in joined or "libelle" in joined)
        and "taille" in joined
        and ("d�but" in joined or "debut" in joined or "deb" in joined)
    )
...
def _parse_field_row(row: tuple, header_offset: int) -> FieldSpec | None:
    ...
    for c in cells[type_idx + 1 :]:
        if isinstance(c, str) and c.strip() in ("O", "F", "C", "Oui", "Non"):
            required = c.strip()
            break
    return FieldSpec(label=label, size=size, start=start, end=end, type_=type_, required=required)


def _parse_sheet(ws, file_field: str, source_file: str) -> FormatSpec2026 | None:
    sheet_name = ws.title.strip()
    # Skip sheets that are clearly not format specs
    if any(skip in sheet_name.lower() for skip in ["pr�sentation", "presentation", "modifications", "lisez"]):
        return None
    ...
    for row in rows:
        for cell in row:
            ...
            if not title and ("Format " in txt or "FICHIER" in txt.upper()):
                title = txt
            if not sector and txt.upper() in (
                "DAF",
                "EX-OQN",
                "OQN",
                "DAF/EX-OQN",
...
def main() -> None:  # pragma: no cover
    ...
    p.add_argument(
        "--src",
        default="C:/Users/adamb/Downloads",
    ...
    for f in files:
        ...
        for s in specs:
            ...
            d["finess_position"] = (s.finess_geo.start, s.finess_geo.size) if s.finess_geo else None
            all_specs.append(d)
