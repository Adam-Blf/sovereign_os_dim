...
#    SOVEREIGN_BRIDGE_TOKEN=secret python -m backend.interfaces.bridge
# ══════════════════════════════════════════════════════════════════════════════
...
from backend.orgchart.structure import parse_structure
...
def create_app() -> Flask:
    ...
    def api_cross_modality():
        ...
        return jsonify(
            _processor.get_cross_modality_patients(
    ...
    def api_chart_from_excel():
        ...
        paths = [p for p in raw_paths if isinstance(p, str) and os.path.isfile(p)]
        if not paths:
        ...
        for filename, buckets in per_file.items():
            datasets.append(
                {
                    "name": filename,
                    "values": [_reduce(buckets.get(lbl, [])) for lbl in ordered_labels],
                }
            )
