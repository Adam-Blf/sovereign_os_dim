...
Lancement ·
    uvicorn backend.interfaces.fastapi_app:app --host 127.0.0.1 --port 8766
═══════════════════════════════════════════════════════════════════════════════
"""
...
@app.get(
    "/api/v2/cockpit", response_model=CockpitResponse, tags=["cockpit"], dependencies=[Depends(require_token)]
)
def cockpit_stats(processor: Annotated[DataProcessor, Depends(get_processor)]):
    ...
    return CockpitResponse(
        ...
        kpis=[
            ...
            CockpitKpi(
                label="Formats actifs", value=str(len(processor.matrix)), sub="ATIH supportés", accent="navy"
            ),
        ],
...
def health_monitor(processor: Annotated[DataProcessor, Depends(get_processor)]):
...
@app.post(
    "/api/v2/ml/predict-format",
    response_model=FormatPredictionResponse,
    tags=["ml"],
    dependencies=[Depends(require_token)],
)
def ml_predict_format(req: FormatPredictionRequest):
    ...
    if "format" not in models:
        raise HTTPException(503, "Modèle format_detector non chargé · lancer backend.ml.train")
...
@app.post(
    "/api/v2/ml/predict-collision-risk",
    response_model=CollisionRiskResponse,
    tags=["ml"],
    dependencies=[Depends(require_token)],
)
...
@app.post(
    "/api/v2/ml/predict-ddn-validity",
    response_model=DdnValidityResponse,
    tags=["ml"],
    dependencies=[Depends(require_token)],
)
...
@app.post(
    "/api/v2/ml/cim-suggest",
    response_model=CimSuggestResponse,
    tags=["ml"],
    dependencies=[Depends(require_token)],
)
def ml_cim_suggest(req: CimSuggestRequest):
    ...
    if parsed.scheme not in ("http", "https"):
        raise HTTPException(503, f"OLLAMA_BASE doit utiliser http ou https, reçu '{parsed.scheme}'")
    ...
    try:
        ...
        sugg = [
            CimSuggestion(**s) for s in parsed[:5] if isinstance(s, dict) and "code" in s and "label" in s
        ]
...
@app.post(
    ...
    response_model=ArsScoreResponse,
    tags=["ars"],
    dependencies=[Depends(require_token)],
)
...
@app.get("/api/v2/audit/verify", tags=["audit"], dependencies=[Depends(require_token)])
def audit_verify():
...
@app.get("/api/v2/idv/collisions", tags=["idv"], dependencies=[Depends(require_token)])
def idv_collisions(
...
@app.get("/api/v2/idv/stats", tags=["idv"], dependencies=[Depends(require_token)])
...
@app.get("/api/v2/cespa/check", tags=["cespa"], dependencies=[Depends(require_token)])
...
@app.get("/api/v2/diff", tags=["diff"], dependencies=[Depends(require_token)])
...
@app.get("/api/v2/heatmap/sectors", tags=["heatmap"], dependencies=[Depends(require_token)])
def heatmap_sectors(processor: Annotated[DataProcessor, Depends(get_processor)]):
    """
...
@app.get("/api/v2/twin/scenarios", tags=["twin"], dependencies=[Depends(require_token)])
def twin_scenarios(processor: Annotated[DataProcessor, Depends(get_processor)]):
    """
...
@app.get("/api/v2/workflow/pending", tags=["workflow"], dependencies=[Depends(require_token)])
...
@app.post("/api/v2/workflow/add", tags=["workflow"], dependencies=[Depends(require_token)])
...
@app.post("/api/v2/workflow/advance/{item_id}", tags=["workflow"], dependencies=[Depends(require_token)])
def workflow_advance(item_id: int, new_stage: Literal["tim", "mim", "preflight", "ars", "done"]):
...
def main() -> None:  # pragma: no cover
    ...
    args = p.parse_args()
    uvicorn.run("backend.interfaces.fastapi_app:app", host=args.host, port=args.port, reload=args.reload)
