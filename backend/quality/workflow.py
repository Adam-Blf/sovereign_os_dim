...
def add_item(ipp: str, label: str, owner: str, stage: Stage = "tim") -> dict:
    ts = _now()
    with _conn() as c:
        cur = c.execute(
            "INSERT INTO items (ipp, label, owner, stage, created, updated) VALUES (?, ?, ?, ?, ?, ?)",
...
def advance(item_id: int, new_stage: Stage) -> dict | None:
    ...
    with _conn() as c:
        ...
        row = c.execute(
            "SELECT id, ipp, label, owner, stage, created, updated FROM items WHERE id = ?", (item_id,)
        ).fetchone()
    if not row:
        return None
    return {
...
def list_pending(stage_filter: Stage | None = None, limit: int = 100) -> list[dict]:
    with _conn() as c:
        if stage_filter:
            rows = c.execute(
...
def stage_counts() -> dict[str, int]:
    ...
    with _conn() as c:
        rows = c.execute("SELECT stage, COUNT(*) FROM items WHERE stage != 'done' GROUP BY stage").fetchall()
    for stage, n in rows:
        if stage in out:
            out[stage] = n
    return out
