...
def append(who: str, action: str, target: str = "") -> dict:
    ...
    with _conn() as c:
        ...
        c.execute(
            "INSERT INTO events (ts, who, action, target, prev_hash, sha256) VALUES (?, ?, ?, ?, ?, ?)",
            (ts, who, action, target, prev, h),
        )
        return {"ts": ts, "who": who, "action": action, "target": target, "prev_hash": prev, "sha256": h}
...
def list_events(limit: int = 30) -> list[dict]:
    ...
    with _conn() as c:
        rows = c.execute(
            "SELECT ts, who, action, target, sha256 FROM events ORDER BY id DESC LIMIT ?", (limit,)
        ).fetchall()
    return [{"ts": r[0], "who": r[1], "action": r[2], "target": r[3], "sha256": r[4]} for r in rows]
...
def verify_chain() -> dict:
    ...
    with _conn() as c:
        rows = c.execute(
            "SELECT id, ts, who, action, target, prev_hash, sha256 FROM events ORDER BY id"
        ).fetchall()
    expected_prev = "0" * 64
    broken_at = None
    for r in rows:
        ...
        if prev != expected_prev:
            broken_at = _id
            break
        recomputed = hashlib.sha256("|".join((prev, ts, who, action, target)).encode()).hexdigest()
        if recomputed != h:
            broken_at = _id
            break
        expected_prev = h
    return {"total_events": len(rows), "valid": broken_at is None, "broken_at_id": broken_at}
