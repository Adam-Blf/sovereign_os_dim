...
fastapi_app = pytest.importorskip("backend.interfaces.fastapi_app")
...
def client():
    ...
    from backend.quality import audit
...
def test_ars_score_empty_when_no_sample(client):
    r = client.post("/api/v2/ars/score-lot", json={"lot_name": "test.txt", "sample_lines": []})
    assert r.status_code == 200
    body = r.json()
    assert body["score"] == 0
...
def test_ars_score_with_real_lines(client):
    line = "X" * 154
    r = client.post("/api/v2/ars/score-lot", json={"lot_name": "test.txt", "sample_lines": [line] * 3})
    assert r.status_code == 200
    body = r.json()
...
def test_auth_blocks_when_token_set(client, monkeypatch):
    ...
    r2 = client.get("/api/v2/cockpit", headers={"Authorization": "Bearer wrong"})
    assert r2.status_code == 403
    r3 = client.get("/api/v2/cockpit", headers={"Authorization": "Bearer secret-test"})
