...
def build(output_path: str) -> str:
    ...
    for i, item in enumerate(QUESTIONS, start=1):
        ...
        pdf.cell(0, 5, f"QUESTION {i:02d} / {len(QUESTIONS)}", new_x="LMARGIN", new_y="NEXT")
        pdf.ln(3)
        # Question (gros titre)
        pdf.set_font(SANS, "B", 24)
        pdf.set_text_color(*GH_NAVY)
        pdf.multi_cell(0, 10, item["q"], new_x="LMARGIN", new_y="NEXT")
        pdf.set_draw_color(*GH_TEAL)
        pdf.set_line_width(0.6)
    ...
    items_after = [
        ...
        (
            ...
            "Document technique pour les informaticiens qui veulent modifier ou étendre le logiciel.",
        ),
        (
