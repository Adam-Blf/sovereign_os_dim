...
def mk(
    ...
    return dict(
        ...
        access=access,
        interface=interface,
        workflow_1=workflow_steps[0],
        workflow_2=workflow_steps[1],
        workflow_3=workflow_steps[2],
        options=options,
    ...
    mk(
        ...
        faq=[
            ...
            (
                ...
                "Oui, via le filtre par format ou en decochant manuellement dans le tableau.",
            ),
        ],
        best_practices=(
    ...
    mk(
        ...
        faq=[
            (
                ...
                "Non encore , prevu pour V36. Utiliser CSV puis convertir en Parquet via script BIQuery.",
            ),
            ("Les exports sont-ils compresses ?", "Non par defaut. Utiliser 7-zip manuellement si besoin."),
            (
    ...
    mk(
        ...
        faq=[
            (
                ...
                "Pas encore , V36 prevoit un selecteur de type (pie, line, bar, area).",
            ),
            (
                "Tableau de bord en direct fonctionne-t-il sans Chart.js ?",
                "Non, Chart.js est embarqué dans l'interface graphique. Disponible hors ligne.",
            ),
            (
                "Puis-je projeter en mode dark pour présentation ?",
                "Oui, le theme suit celui de l'app. Utiliser Ctrl+D pour basculer.",
            ),
            (
    ...
    mk(
        ...
        faq=[
            (
                ...
                "Non, c'est une vue de consultation. Editer le CSV avec Excel puis recharger.",
            ),
            (
                "Combien de niveaux de profondeur sont supportes ?",
                "Jusqu'a 10, au-dela l'affichage devient illisible.",
            ),
            ("Le PDF est-il vectoriel ?", "Oui, pas de raster. Qualité impression optimale même agrandi."),
            ("Puis-je exporter en SVG ?", "Pas encore. L'organigramme est HTML+CSS pur, pas de SVG."),
        ],
        best_practices=(
    ...
    mk(
        ...
        faq=[
            ...
            (
                ...
                "Oui, charger le fichier structure de l'etablissement concerne. L'outil est generique.",
            ),
            (
    ...
    mk(
        ...
        faq=[
            ...
            (
                ...
                "Non par sécurité , uniquement fichiers locaux. Telecharger d'abord, puis importer.",
            ),
            (
    ...
    mk(
        ...
        faq=[
            (
                ...
                "GitHub Issues , https://github.com/Adam-Blf/sovereign_os_dim/issues ou email direct.",
            ),
            (
                "Comment je propose une amelioration ?",
                "Email avec titre '[FR] ...' ou issue GitHub avec label 'enhancement'.",
            ),
            ("Le code est-il open source ?", "Oui, MIT + LGPL (fpdf2). Contributions bienvenues."),
            (
...
def _page_header(pdf, logo_path, feat_title, category, section_idx, total_sections):
    ...
    pdf.cell(
        120, 4, f"{category}, section {section_idx:02d} sur {total_sections}", new_x="RIGHT", new_y="TOP"
    )
...
def _feature_schema(pdf, feat_num):
    ...
    titles = {
        1: "TABLEAU DE BORD",
        2: "SELECTION FICHIERS",
        3: "IDENTITOVIGILANCE",
        4: "PMSI PILOT CSV",
        5: "INSPECTEUR + CONTROLE PREALABLE",
        6: "TABLEAU DE BORD LIVE",
        7: "STRUCTURE",
        8: "ANALYSE D'ACTIVITE UM",
        9: "IMPORT CSV + HTML->PDF",
        10: "ADMINISTRATION",
    }
    ...
    if feat_num == 1:
        ...
        for i in range(36):
            ...
            pdf.set_fill_color(*color)
            pdf.circle(
                center[0] + radius * math.cos(rad) * 0.6, center[1] + radius * math.sin(rad) * 0.6, 1.5, "F"
            )
        pdf.set_font(SANS, "", 7)
        pdf.set_text_color(*SLATE_500)
...
def render_feature(pdf, feat, logo_path, feat_num, total_feats, screenshot_path=None):
    ...
    pdf.cell(
        0,
        4,
        f"FONCTIONNALITE {feat_num:02d} / {total_feats}  ,  {cat.upper()}",
        new_x="LMARGIN",
        new_y="NEXT",
    )
...
def build_pdf(output_path: str) -> str:
    ...
    if not unicode_ok:  # pragma: no cover
        print(
            "[WARN] Police TTF Unicode introuvable , accents desactives. "
            "Installer DejaVu (Linux , apt install fonts-dejavu) ou copier "
            "les TTF dans tools/fonts/ pour reactiver.",
            file=sys.stderr,
        )
    ...
    # ══════ BLOC INTRODUCTION ══════
    pdf.add_page()
    _page_header(pdf, LOGO_PATH, "Introduction du guide", "Introduction", 0, total_feats)
    ...
    for page_idx in range(0, len(sentinel_gallery), 4):
        ...
        pdf.add_page()
        _page_header(pdf, LOGO_PATH, "Galerie des vues V36", "Sentinel , Refonte écrans", 0, total_feats)
        _page_title(pdf, page_idx // 4 + 1, f"Galerie des écrans , {page_idx + 1}/{len(sentinel_gallery)}")
        _body_text(
            pdf,
        ...
        for idx, (png, title, sub) in enumerate(page_items):
            ...
            if os.path.exists(png_path):
                try:
                    pdf.image(png_path, x=x0 + 0.5, y=y0 + 0.5, w=cell_w - 1, h=cell_h - 1)
